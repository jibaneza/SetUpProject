...
function findproject(dir::AbstractString = pwd())
    ...
    @warn "SetUpProject could not find find a project file by recursively checking "*
...
!!! warning
    Note that to access `quickactivate` you need to be `using SetUpProject`.
    For this to be possible `SetUpProject` must be already added in the
    existing global environment. If you use `quickactivate` and share your project, do
    note to your co-workers that they need to add `SetUpProject` globally (the default
    README.md created by `initialize_project` says this automatically).

    **In addition, in your scripts write:**
    ```julia
    using SetUpProject # YES
    ...
    **instead of the erroneous:**
    ```julia
    using SetUpProject, Package1, Package2 # NO!
    quickactivate(@__DIR__)
    # do stuff
    ```
    This ensures that the packages you use will all have the versions dictated
    by your activated project (besides `SetUpProject`, since this is impossible
...
`SetUpProject` and use `@quickactivate` in different `begin` blocks as follows:
```julia
using Distributed
addprocs(8)
@everywhere using SetUpProject
...
Initialize a scientific project expected by `SetUpProject` in `path` (directory representing
an empty folder).
...
* `template = SetUpProject.DEFAULT_TEMPLATE` : A template containing the folder structure
...
function initialize_project(path, name = default_name_from_path(path);
    ...
    if git
        ...
        LibGit2.get(gc, "user.name", LibGit2.getconfig("user.name", false)) ||
            LibGit2.set!(gc, "user.name", "SetUpProject")
    ...
    git && LibGit2.commit(repo, "Folder setup by SetUpProject")
    ...
    git && LibGit2.commit(repo, "File setup by SetUpProject")
    return path
end
...
function compat_entry()
    SetUpProject_VERSION = let
        project = joinpath(dirname(dirname(pathof(SetUpProject))), "Project.toml")
    ...
    SetUpProject = "$(SetUpProject_VERSION)"
    """
end
...
function DEFAULT_README(name, authors = nothing)
    ...
    This code base is using the Julia Language and [SetUpProject](https://juliadynamics.github.io/SetUpProject.jl/stable/)
    ...
    1. Open a Julia console and do:
       ```
       julia> using Pkg
       julia> Pkg.add("SetUpProject") # install globally, for using `quickactivate`
